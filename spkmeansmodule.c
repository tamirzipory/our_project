...
static PyObject *DiagonalDegreeMatrix(PyObject *self, PyObject *args)
{
    ...
    print_err_with_assertg(weightedAdjMatrix != NULL);
    for (i = 0; i < m; i++)
    {
        weightedAdjMatrix[i] = (double *)calloc(n, sizeof(double));
        assert(weightedAdjMarixt[i] != NULL); /* notice assert not print_err_with_assertg because of compiler */
    }
...
static PyObject *Jacobi(PyObject *self, PyObject *args)
{
    ...
    print_err_with_assertg(A != NULL);
    for (i = 0; i < m; i++)
    {
        A[i] = (double *)calloc(n, sizeof(double));
        print_err_with_assertg(A[i] != NULL);
    }
...
static PyMethodDef spkmeansMethods[] = {
    ...
    {"Jacobi",
     (PyCFunction)Jacobi,
     METH_VARARGS,
     PyDoc_STR("Calculation of eigenvalues and pairs with the Jacobi algorithm (1.2.1)")},
    {"heuristic",
